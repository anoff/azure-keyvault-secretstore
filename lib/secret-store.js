class SecretStore {
  ...
  refresh (keyVaultClient) {
    if (!keyVaultClient || typeof keyVaultClient.getSecret !== 'function') {
      throw new Error('invalid keyVaultClient')
    }
    // prevent update process from triggering multiple times concurrently
    if (!this.isUpdating) {
      this.isUpdating = true
      this._updatePromise = Promise.all(this.secrets.map(s => {
