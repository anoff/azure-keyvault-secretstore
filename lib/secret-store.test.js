...
test('get() returns correct object', t => {
  const s = new SecretStore(vaultUrl, keyvaultFn)
  s.add('a').add('b').add('c')
  const v = s.get('b')
  t.is(v.name, 'b')
  t.falsy(s.get('notHere'), 'non existant lookup returns falsy value')
})
...
test('refresh() should update all secrets', async t => {
  ...
  s.add('b')
  await s.refresh()
  t.is(s.get('a').value, 'a-fancyValue', 'secret value is updated')
})
...
test('refresh() should not execute concurrent updates', async t => {
  ...
  t.is(s.get('a').value, 'a-0', 'returns initial callCount after multiple refresh() calls')
})
